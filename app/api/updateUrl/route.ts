...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!oldShortUrl || !newShortUrl) {
      return NextResponse.json({ message: 'Old and new short URLs must be provided' }, { status: 400 });
    }
    ...
    if (existingUrlRecord) {
      return NextResponse.json({ message: 'New short URL already exists' }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: 'Short URL updated successfully', updatedUrlRecord });
  } catch (error) {
